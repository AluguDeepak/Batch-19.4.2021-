...
namespace ProductAPITestUsingMoq.Xunit
{
    public class TestProductRepo
    {
        ...
        public TestProductRepo()
        {
            ...
            mockRepo.Setup(repo => repo.GetProducts()).Returns(products.ToList());
            ...
            mockRepo.Setup(repo => repo.UpdateProduct(It.IsAny<Products>())).Callback((Products item) =>
            {
                item = new Products() { Pid = 6, Pname = "DDD", Price = 120, Stock = 100 };
                products.Remove(item);
                products.Add(item);
            }).Verifiable();
            mockRepo.SetupAllProperties();
            _repo = mockRepo.Object; //to initiate Product repo object
        }
        [Fact]
        ...
        {
            var productdetails = new Products() { Pid = 5, Pname = "CCC", Price = 120, Stock = 100 };
